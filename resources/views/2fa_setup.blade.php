...
@section('content')
    <div class="card p-4">
        ...
        @if(auth()->user()->two_factor_enabled_at)
            ...
            <form method="POST" action="{{ route('2fa.disable') }}" class="text-center">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger">Disable 2FA</button>
            </form>

        @elseif($qrCodeUrl ?? false)
            <div class="alert alert-info text-center">
                ...
                <img src="https://api.qrserver.com/v1/create-qr-code/?data={{ urlencode($qrCodeUrl) }}&size=200x200"
                     alt="2FA QR Code" class="mx-auto d-block mb-3">
                <p>Or enter this secret manually: <code>{{ $secret }}</code></p>
            </div>
            <form method="POST" action="{{ route('two-factor.setup') }}" class="text-center">
                @csrf
